...
@testset "CEP" begin
    using Clp
    using ClustForOpt
    @testset "CEP TX_1" begin #Compare to Merrick Testcase
        # load data
    ...
    @testset "CEP Seasonal and Simple" begin #Check if seasonal with 24h and 365days has the same result as simple with 8760h in one period
        # load data
    ...
    @testset "CEP $state" for (state, years) in [["GER_1", [2016]], ["CA_1", [2016]]] begin
        # laod data
        ...
        @test run_opt(ts_clust_data.best_results,cep_data,optimizer;descriptor="design&operation", co2_limit=50).status=="OPTIMAL"
